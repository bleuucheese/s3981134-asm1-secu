...
class HomomorphicAnalyticsBenchmark:
    """Benchmark homomorphic encryption for analytics tasks."""
    ...
    def encrypt_mean_variance(self, telemetry_data: np.ndarray) -> Dict:
        ...
        errors = [abs(p - r) for p, r in zip(plaintext_means, plaintext_means_ref)]
    ...
    def run_benchmarks(self, data_dir: Path):
        ...
        try:
            ...
            if 'error' not in result:
                ...
                print(f" Encrypt median: {enc_stats['median_s']*1e3:.3f} ms")
                print(f" Compute median: {comp_stats['median_s']*1e3:.3f} ms")
                print(f" Decrypt median: {dec_stats['median_s']*1e3:.3f} ms")
                print(f" Max error: {result['max_error']:.6f}")
                print(f" Relative error: {result['relative_error']:.6f}")
                print(f" Expansion factor: {result['expansion_factor']:.2f}x")
            else:
